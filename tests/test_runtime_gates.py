...
class RuntimeGateTests(unittest.TestCase):
    def test_legacy_helper_uses_exact_pin_and_supported_node_floor(self):
        ...
        self.assertIn('[string]$OpenClawTag = "2026.8.2"', script)
    ...
    def test_ci_enforces_openclaw_node_range_in_both_jobs(self):
        ...
        self.assertGreaterEqual(workflow.count("is unsupported by OpenClaw 2026.8.2"), 2)

    def test_release_enforces_openclaw_node_range(self):
        workflow = (ROOT / ".github" / "workflows" / "release.yml").read_text(encoding="utf-8")

        self.assertIn("22.22.3", workflow)
        self.assertIn("is unsupported by OpenClaw 2026.8.2", workflow)
